...
function handleWebpageOpen(request, response) {
  ...
  page.onResourceRequested = function(requestData, networkRequest) {
    if ((/http:\/\/.+?\.css(\?.+)?$/gi).test(requestData['url'])) {
      request.abort();
    }else if ((/http:\/\/.+?\.png(\?.+)?$/gi).test(requestData['url'])) {
      request.abort();
    }else if ((/http:\/\/.+?\.gif(\?.+)?$/gi).test(requestData['url'])) {
      request.abort();
    }else if ((/http:\/\/.+?\.jpeg(\?.+)?$/gi).test(requestData['url'])) {
      request.abort();
    }else if ((/http:\/\/.+?\.jpg(\?.+)?$/gi).test(requestData['url'])) {
      request.abort();
    }
  };
}
